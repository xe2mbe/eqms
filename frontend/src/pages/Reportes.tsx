...
const ALL_COL_KEYS = [
  'id', 'indicativo', 'operador', 'senal', 'ciudad', 'pais', 'estado', 'zona',
...
export default function ReportesPage() {
  ...
  const { colOrder, colVisible, colSettingsButton } = useColPrefs(
    'reportes_v5', user?.id, ALL_COL_KEYS, LOCKED_KEYS, COL_LABELS,
  )
  ...
  const actionCol: TableColumnType<Reporte> = {
      ...
      <Card className="card-shadow" style={{ marginBottom: 16 }}>
        <Space wrap>
          ...
          <Select
            ...
            options={sistemas.map(s => ({ value: s.id, label: s.codigo }))}
            onChange={(v) => setTempFilters(prev => ({ ...prev, sistema_id: v }))}
          />
          <Input
